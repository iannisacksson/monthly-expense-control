...
export class BulkDeleteExpensesUseCase {
  constructor(
  ...
  async execute(
    ...
    const foundExpenses = await this.expenseRepository.findByIds(
      expenses.map((e) => e.id),
    );
    ...
    const allBelongToMonth = foundExpenses.every(
    ...
    await this.expenseRepository.deleteMany(foundExpenses);
  }
}

...
export class IncomeTaxationService {
  normalizeTaxation(taxation?: IncomeTaxationDTO | null): NormalizedIncomeTaxation {
    if (!taxation || taxation.mode === "manual") {
      return {
        mode: "manual",
        profile: null,
        parameters: null,
  ...
  calculateAutomaticTaxes(grossIncome: number, taxation: NormalizedIncomeTaxation): CalculatedIncomeTax[] {
    ...
    const proLaboreRate = Number(parameters.pro_labore_rate ?? DEFAULT_PRO_LABORE_RATE)
    ...
    const irrf = irrfMode === "manual_amount"
    ...
    return [
      ...
      { tax_type: "CONTADOR", value: this.roundMoney(accountantFee), is_auto: true },
    ]
  }

  private normalizeMeProLaboreParameters(parameters?: MeProLaboreTaxationParametersDTO | null) {
    if (!parameters) {
      throw new Error("Taxation parameters are required for the selected profile")
    }
    ...
    const inssRate = Number(parameters.inss_rate ?? DEFAULT_INSS_RATE)
    const irrfMode = parameters.irrf_mode ?? "disabled"
    const irrfManualAmount = parameters.irrf_manual_amount !== undefined
    ...
    if (irrfMode === "manual_amount") {
      if (irrfManualAmount === undefined || !Number.isFinite(irrfManualAmount) || irrfManualAmount < 0) {
        throw new Error("IRRF manual amount must be greater than or equal to zero")
      }
    }
    ...
    return {
      ...
      inss_rate: inssRate,
      irrf_mode: irrfMode,
      irrf_manual_amount: irrfMode === "manual_amount" ? this.roundMoney(irrfManualAmount ?? 0) : undefined,
    }
  }

...
describe("income tax use cases", () => {
  ...
  it("updates a manual tax for the owner", async () => {
    ...
    await expect(
      useCase.execute("tax-1", { value: 20 }, "user-1"),
    ).resolves.toBe(updatedTax);
  });
  ...
  it("rejects manual update of an automatic tax", async () => {
    ...
    await expect(
      useCase.execute("tax-1", { value: 20 }, "user-1"),
    ).rejects.toThrow("Automatic income taxes cannot be edited manually");
  });

  it("deletes a manual tax for the owner", async () => {
    const existingTax = makeTax("income-1", false);
    const useCase = new DeleteIncomeTaxUseCase(
      {
        findById: vi.fn().mockResolvedValue(existingTax),
        delete: vi.fn().mockResolvedValue(undefined),
      },
      { findById: vi.fn().mockResolvedValue(makeIncome("user-1")) },
    );

    await expect(useCase.execute("tax-1", "user-1")).resolves.toBe(existingTax);
  });
});

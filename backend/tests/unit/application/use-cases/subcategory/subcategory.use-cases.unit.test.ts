...
function makeSubcategory(categoryId = "category-1") {
  return {
    getDataValue: vi.fn((field: string) => {
      if (field === "category_id") return categoryId
      return undefined
    }),
  }
}
...
describe("subcategory use cases", () => {
  it("creates a subcategory when the category belongs to the requester", async () => {
    ...
    const subcategoryRepository = { create: vi.fn().mockResolvedValue(createdSubcategory) }
    const categoryRepository = { findById: vi.fn().mockResolvedValue(makeCategory("user-1")) }
    const useCase = new CreateSubcategoryUseCase(subcategoryRepository, categoryRepository)
  ...
  it("lists subcategories by category for the owner", async () => {
    ...
    await expect(useCase.execute("category-1", "user-1")).resolves.toBe(subcategories)
  })
  ...
  it("returns a subcategory by id for the owner", async () => {
    ...
    await expect(useCase.execute("subcategory-1", "user-1")).resolves.toBe(subcategory)
  })
  ...
  it("deletes a subcategory when it belongs to the requester", async () => {
    const deletedSubcategory = { id: "subcategory-1" }
    const useCase = new DeleteSubcategoryUseCase(
      {
        findById: vi.fn().mockResolvedValue(makeSubcategory("category-1")),
        delete: vi.fn().mockResolvedValue(deletedSubcategory),
      },
      { findById: vi.fn().mockResolvedValue(makeCategory("user-1")) },
    )

    await expect(useCase.execute("subcategory-1", "user-1")).resolves.toBe(deletedSubcategory)
  })
})